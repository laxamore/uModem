...
static void tx2_task(void* args) {
  ...
  while (1) {
    if (osMessageQueueGet(tx2_queue, &data, NULL, osWaitForever) == osOK) {
      if (osSemaphoreAcquire(tx2_sem, osWaitForever) != osOK) continue;
...
static void process_dma_data(void) {
  ...
  if (end > start)
    ...
    umodem_buffer_push(
        (const uint8_t*)&dma_rx2_buffer[start], DMA_RX_BUFFER_SIZE - start);
...
void umodem_hal_init(void) {
  ...
  if (!tx2_sem || !tx2_queue) Error_Handler();
  ...
  if (osThreadNew(tx2_task, NULL, &tx2TaskAttr) == NULL) Error_Handler();
...
int umodem_hal_send(const uint8_t* buf, size_t len) {
  ...
  tx_rx_data txrx = {0};
  txrx.bytes = pvPortMalloc(len);
  memset(txrx.bytes, 0, len);
  if (txrx.bytes == NULL) return -1;
  memcpy(txrx.bytes, buf, len);
  txrx.len = len;
  if (osMessageQueuePut(tx2_queue, &txrx, 0, 0) != osOK) {
    vPortFree(txrx.bytes);
    return -1;
  }
  return len;
}

int umodem_hal_read(uint8_t* buf, size_t len) { return 0; }

uint32_t umodem_hal_millis(void) {
  return osKernelGetTickCount() * portTICK_PERIOD_MS;
}

void umodem_hal_delay_ms(uint32_t ms) { osDelay(pdMS_TO_TICKS(ms)); }

void umodem_hal_lock(void) { osSemaphoreAcquire(tx2_sem, osWaitForever); }

void umodem_hal_unlock(void) { osSemaphoreRelease(tx2_sem); }

// --- Callbacks ---
void HAL_UART_RxHalfCpltCallback(UART_HandleTypeDef* huart) {
  if (huart->Instance == USART2) process_dma_data();
}
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef* huart) {
  if (huart->Instance == USART2) {
    ...
    if (HAL_UART_Receive_DMA(
            &huart2, (uint8_t*)dma_rx2_buffer, DMA_RX_BUFFER_SIZE) != HAL_OK)
      Error_Handler();
...
void* umodem_hal_alloc(size_t size) { return pvPortMalloc(size); }

void umodem_hal_free(void* ptr) { vPortFree(ptr); }