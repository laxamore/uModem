#ifndef uMODEM_CORE_H_
#define uMODEM_CORE_H_
...
static inline int umodem_strtoi(
    const char* str, int min_val, int max_val, int* out) {
  if (!str) return 0;
  char* endptr;
  errno = 0;
  long val = strtol(str, &endptr, 10);
  if (errno != 0 || endptr == str) return 0; // invalid input
...
static inline uint32_t umodem_rand(void) {
  ...
  if (first_call) {
    int stack_var;
    extern uint32_t umodem_hal_millis(void);
